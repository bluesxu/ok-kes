...
- **加速模式**（实验性，默认关闭）：点击后页面一有响应就继续，不再按固定时长等待；路线页图标停稳即开始识别，牌库每次翻一整行，多种图标并行匹配。游戏自己的战斗速度不受影响。关闭后完全使用原逻辑。
...
- **目标卡删完后不再整库翻找**：需开启加速模式，并关闭“优先移除基础牌”和“刷空档”。某一局整库确认已没有“移除卡牌列表”里的卡后，这局再遇到删卡只看最底页（仍会删咒术卡）就跳过，事件里含“移除”的任务优先级也会排到最后；换一局后重新检查。
